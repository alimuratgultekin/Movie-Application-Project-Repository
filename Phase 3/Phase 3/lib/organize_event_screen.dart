...
class _OrganizeEventScreenState extends State<OrganizeEventScreen> {
  ...
  Future<void> _createEvent() async {
    if (_formKey.currentState!.validate() && _selectedMovie != null) {
      ...
      // Find movie title from selected movie ID
      final movies = Movie.getTrendingMovies();
      final selectedMovie = movies.firstWhere((m) => m.id == _selectedMovie);
      final movieTitle = selectedMovie.title;
      ...
      final eventId = await eventProvider.createEvent(
        ...
        organizerName: authProvider.displayName,
      );
        ...
        if (context.mounted) {
          showDialog(
            ...
            builder: (context) => AlertDialog(
              ...
              content: Text(
                eventProvider.error ?? 'Failed to create event. Please try again.',
                style: AppTextStyles.bodyMedium,
              ),
              actions: [
                TextButton(
