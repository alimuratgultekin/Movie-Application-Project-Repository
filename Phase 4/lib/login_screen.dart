...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void initState() {
    super.initState();
    // Listen to form field changes to update button state
  ...
  Future<void> _handleLogin() async {
    // Validate form - if invalid, show inline errors (no popup)
    if (!_formKey.currentState!.validate()) {
      return; // Inline validation errors are already displayed
    }
    ...
    if (success) {
      ...
      // Show error dialog only for Firebase/auth errors
      if (context.mounted) {
        showDialog(
          context: context,
          builder: (context) => AlertDialog(
            ...
            content: Text(
              authProvider.error ?? 'Please check your email and password and try again.',
  ...
  Future<void> _handleDemoMode() async {
    // For demo mode, we'll still use Firebase but with a demo account
    // You can create a demo account in Firebase or use a test account
    final authProvider = Provider.of<AuthProvider>(context, listen: false);
    ...
    // Try to sign in with demo credentials (you should create this account in Firebase)
    final success = await authProvider.signIn(
    ...
    if (success) {
      ...
      if (context.mounted) {
        showDialog(
          ...
          builder: (context) => AlertDialog(
            backgroundColor: AppColors.cardBackground,
            title: const Text('Demo Mode', style: AppTextStyles.h3),
            content: const Text(
              'Demo account not found. Please create an account or sign in with your credentials.',
              style: AppTextStyles.bodyMedium,
            ),
            actions: [
              TextButton(
            ...
            onPressed: () async {
              if (formKey.currentState!.validate()) {
                ...
                if (success) {
                  // Show success dialog
                  if (context.mounted) {
                    Navigator.pop(context); // Close create account dialog
                    showDialog(
                      context: context,
                      builder: (context) => AlertDialog(
                        backgroundColor: AppColors.cardBackground,
                  ...
                  if (context.mounted) {
                    showDialog(
                      ...
                      builder: (context) => AlertDialog(
                        backgroundColor: AppColors.cardBackground,
                        title: const Text('Sign Up Failed', style: AppTextStyles.h3),
                        content: Text(
